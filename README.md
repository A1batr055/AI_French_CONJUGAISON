...
声明：本仓库的 index.html 以及本 README 由 AI 生成。
模型限定：本项目仅使用 Google Gemini「免费」模型 gemini-2.0-flash。
...
功能
	...
	•	练习出题（固定规则）：选择「语言水平 → 练习时态 → 动词来源（分组 / 全库随机）」，开始后每个动词出 6 题（六个人称：Je、Tu、Il/Elle/On、Nous、Vous、Ils/Elles）。总题数 = 词数 × 6。
...
获取 API Key（免费）
	...
	3.	回到本页 设置 粘贴保存即可。
	4.	不要修改源码里的模型名，本项目固定走：
gemini-2.0-flash:generateContent（已在代码中写好）。

⸻

隐私与安全
	•	你的 API Key 不会上传：仅存储在浏览器 localStorage。
	•	不要在公共电脑上长期保存 Key。建议为此项目单独生成可随时停用的 Key。
	•	项目为前端应用，访问者可“查看源代码”。
  
⸻

  常见问题（超简版）
	•	400 / Key not valid：Key 前后多了空格或换行；重新粘贴保存。
